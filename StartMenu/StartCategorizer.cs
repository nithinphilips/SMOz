...
//						  Debug.WriteLine("[Not In Root: " + (forceAllFilters ? "OK]" : "SKIP]") + currentItem.Name);
					   } else {				  
						  // never applied
		  ...
		  //    StartItem currentItem = items[i];
		  //    Template.Category newCategory = template.FindCategory(currentItem.Name);
		  //    if (newCategory == null) { continue; }

		  //    if (rootAndRestricted) {
		  //        // If Item's category is not empty or category is not the restricted
		  //        if ((currentItem.Category != "") && !currentItem.Name.StartsWith(newCategory.RestrictedPath, true, CultureInfo.CurrentCulture)) {
		  //            Console.WriteLine("Skipping " + currentItem.Name);
		  //            continue; 
		  //        }
		  //    } else {
		  //        if (!forceAllFilters && (string.Compare(newCategory.RestrictedPath, category, true) != 0)) {
		  //            Console.WriteLine("Skipping " + category);
		  //            continue;
		  //        }	
		  //    }

		  //    if (string.Compare(currentItem.Category, newCategory.Name, true) != 0) {
		  //        result.Add(new MoveStartItemCommand(currentItem, newCategory.Name));
		  //    }
		  //}
